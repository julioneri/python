num = int(input('Digite algum número: '))
print('\033[1;4mMULTIPLICAÇÃO DO NÚMERO', num, '\033[m')
print('='*11)
print('{} x  1 ='.format(num), num * 1)
print('{} x  2 ='.format(num), num * 2)
print('{} x  3 ='.format(num), num * 3)
print('{} x  4 ='.format(num), num * 4)
print('{} x  5 ='.format(num), num * 5)
print('{} x  6 ='.format(num), num * 6)
print('{} x  7 ='.format(num), num * 7)
print('{} x  8 ='.format(num), num * 8)
print('{} x  9 ='.format(num), num * 9)
print('{} x 10 ='.format(num), num * 10)
print('='*11)
print('\033[1;4mDIVISÃO DO NÚMERO', num, '\033[m')
print('{} / 1 ='.format(num), num / 1)
print('{} / 2 ='.format(num), num / 2)
print('{} / 3 ='.format(num), num / 3)
print('{} / 4 ='.format(num), num / 1)
print('{} / 5 ='.format(num), num / 2)
print('{} / 6 ='.format(num), num / 3)
print('{} / 7 ='.format(num), num / 1)
print('{} / 8 ='.format(num), num / 2)
print('{} / 9 ='.format(num), num / 3)
print('{} / 10 ='.format(num), num / 1)